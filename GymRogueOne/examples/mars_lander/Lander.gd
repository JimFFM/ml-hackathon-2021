...
func impulse(engine_id : String) -> void:
	...
	if engine_id == "AuxX":
		...
		apply_impulse(transform.basis.xform($MeshFlameMain.transform.origin), 4*transform.basis.y*jetStrength*hoverStrength)
	elif engine_id == "None":
		pass

func mainengine(strength: float) -> void:
